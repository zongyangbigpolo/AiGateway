...
object AngConfigManager {
    ...
    private fun parseConfig(
        ...
        try {
            ...
            if (logErrors) LogUtil.e(AppConfig.TAG, "Failed to parse config", e)
            return null
        }
    }

    @Synchronized
    fun importInvitationNodes(content: String, subid: String): Int {
        val configs = InvitationProtocol.nodeLines(content).reversed().map {
            parseConfig(it, subid, null, logErrors = false)
                ?: throw InvitationException(InvitationError.RESPONSE)
        }
        if (configs.any { it.server.isNullOrBlank() || it.serverPort?.toIntOrNull() !in 1..65535 }) {
            throw InvitationException(InvitationError.RESPONSE)
        }
        val filter = MmkvManager.decodeSubscription(subid)?.filter
        val filtered = if (filter.isNullOrBlank()) configs else {
            val regex = try {
                Regex(filter)
            } catch (_: java.util.regex.PatternSyntaxException) {
                throw InvitationException(InvitationError.RESPONSE)
            }
            configs.filter { it.remarks.isEmpty() || regex.containsMatchIn(it.remarks) }
        }
        if (filtered.isEmpty()) throw InvitationException(InvitationError.RESPONSE)
        val selected = getRemovedSelectedProfile(subid, false)
        MmkvManager.removeServerViaSubid(subid)
        val saved = batchSaveConfigs(filtered, subid)
        findMatchedProfileKey(saved, selected)?.let { MmkvManager.setSelectServer(it) }
        return filtered.size
    }

    private fun updateInvitationSubscription(it: SubscriptionCache): SubscriptionUpdateResult {
        return try {
            val content = runBlocking(Dispatchers.IO) {
                val origin = InvitationAccess.subscriptionOrigin(it.subscription)
                val api = InvitationApi()
                api.validate(origin, it.subscription.url)
                api.subscription(origin, it.subscription.url)
            }
            val count = importInvitationNodes(content, it.guid)
            it.subscription.lastUpdated = System.currentTimeMillis()
            MmkvManager.encodeSubscription(it.guid, it.subscription)
            SubscriptionUpdateResult(configCount = count, successCount = 1)
        } catch (_: IOException) {
            LogUtil.w(AppConfig.TAG, "Invitation subscription update failed; existing nodes retained")
            MessageUtil.sendMsg2Service(
                AngApplication.application, AppConfig.MSG_INVITATION_AUTHORIZATION_FAILED, it.guid
            )
    ...
    fun updateConfigViaSub(it: SubscriptionCache): SubscriptionUpdateResult {
        if (!it.subscription.enabled || it.guid.isBlank() || it.subscription.url.isBlank()) {
            return SubscriptionUpdateResult(skipCount = 1)
        }
        return updateInvitationSubscription(it)
    }
