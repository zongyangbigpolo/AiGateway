...
object SettingsManager {
    ...
    private fun initRoutingRulesets(context: Context) {
        // An empty saved list is intentional; never overwrite it on subsequent launches.
        if (MmkvManager.decodeSettingsString(AppConfig.PREF_ROUTING_RULESET) == null) {
            val rules = context.assets.open(GatewayRuleFile.DEFAULT_ASSET).use(GatewayRuleFile::read)
            check(MmkvManager.encodeSettings(AppConfig.PREF_ROUTING_RULESET, JsonUtil.toJson(rules))) {
                "Failed to save default gateway rules"
            }
        }
    }

    fun getGatewayDefaultOutbound(): String =
        MmkvManager.decodeSettingsString(AppConfig.PREF_GATEWAY_DEFAULT_OUTBOUND) ?: TAG_DIRECT

    fun importGatewayRules(rules: List<RulesetItem>): Boolean {
        val locked = MmkvManager.decodeRoutingRulesets().orEmpty().filter { it.locked == true }
        val saved = MmkvManager.encodeSettings(
            AppConfig.PREF_ROUTING_RULESET,
            JsonUtil.toJson(locked + rules.filterNot { it in locked })
        )
        if (saved) SettingsChangeManager.makeRestartService()
        return saved
    }
    ...
    fun resetRoutingRulesetsFromPresets(context: Context, index: Int) {
        ...
        MmkvManager.encodeSettings(AppConfig.PREF_GATEWAY_DEFAULT_OUTBOUND, AppConfig.TAG_PROXY)
    }
    ...
    fun initAssets(context: Context, assets: AssetManager) {
        ...
        if (extFolder.isBlank()) throw IOException("Cannot locate rule directory.")
        GatewayGeoFiles.installMissing(File(extFolder)) { assets.open(it) }
    }
    ...
    private fun ensureDefaultSettings() {
        ...
        ensureDefaultValue(AppConfig.PREF_GATEWAY_DEFAULT_OUTBOUND, TAG_DIRECT)
        ensureDefaultValue(AppConfig.PREF_GEO_FILES_SOURCES, AppConfig.GEO_FILES_SOURCES.first())
        ensureDefaultValue(AppConfig.PREF_ROUTING_DOMAIN_STRATEGY, "IPIfNonMatch")
        ensureDefaultValue(AppConfig.PREF_VPN_BYPASS_LAN, "2")
        if (!MmkvManager.hasSettings(AppConfig.PREF_LOCAL_DNS_ENABLED)) {
            MmkvManager.encodeSettings(AppConfig.PREF_LOCAL_DNS_ENABLED, true)
        }
        if (!MmkvManager.hasSettings(AppConfig.PREF_APPEND_HTTP_PROXY)) {
