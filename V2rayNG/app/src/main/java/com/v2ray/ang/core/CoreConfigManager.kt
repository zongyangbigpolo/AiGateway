...
object CoreConfigManager {
    ...
    fun getV2rayConfig4Speedtest(context: Context, guid: String): ConfigResult {
        try {
            ...
            val v2rayConfig = buildUnifiedConfig(configContext, AppConfig.TAG_PROXY)
     ...
     */
    private fun buildUnifiedConfig(
        configContext: CoreConfigContext,
        defaultOutbound: String = SettingsManager.getGatewayDefaultOutbound()
    ): V2rayConfig {
        ...
        GatewayRouting.applyDefault(
            v2rayConfig,
            defaultOutbound,
            policyGroupBalancerTags[AppConfig.TAG_PROXY]
        )
    ...
    */
    ...
    private fun configureDns(
        ...
        val defaultDirect = SettingsManager.getGatewayDefaultOutbound() == AppConfig.TAG_DIRECT
        val defaultDnsTags = mutableListOf<String>()
        if (defaultDirect) {
            domesticDns.forEachIndexed { index, address ->
                val tag = "${AppConfig.TAG_DOMESTIC_DNS}_default_$index"
                servers.add(V2rayConfig.DnsBean.ServersBean(address = address, tag = tag))
                defaultDnsTags.add(tag)
            }
        } else {
            remoteDns.forEach { servers.add(it) }
        }

        val hosts = buildDnsHosts()
        ...
        domesticDnsTags.addAll(defaultDnsTags)
    ...
    private fun buildDnsHosts(): MutableMap<String, Any> {
        val hosts = mutableMapOf<String, Any>()
        // Enforce blocks in ordered routing rules, not hosts overrides that bypass earlier allow rules.
