...
class UserAssetAdapter(
    ...
    override fun onBindViewHolder(holder: UserAssetViewHolder, position: Int) {
        ...
        holder.itemUserAssetBinding.assetSource.text = if (item.assetUrl.url == "file") {
            holder.itemUserAssetBinding.root.context.getString(R.string.geo_source_local)
        } else {
            item.assetUrl.url
        }
        ...
        holder.itemUserAssetBinding.layoutEdit.visibility = View.VISIBLE
        holder.itemUserAssetBinding.layoutRemove.visibility =
            if (item.assetUrl.remarks in GatewayGeoFiles.names) View.GONE else View.VISIBLE
