    ...
    fun downloadToFile(
        ...
        return try {
            client.newCall(requestBuilder.build()).execute().use { response ->
                ...
                if (body.contentLength() > maxBytes) throw IOException("Download exceeds size limit")
                body.byteStream().use { input ->
                    targetFile.outputStream().use { output ->
                        val buffer = ByteArray(DEFAULT_BUFFER_SIZE)
                        var total = 0L
                        while (true) {
                            if (Thread.currentThread().isInterrupted) throw InterruptedIOException("Download cancelled")
                            val count = input.read(buffer)
                            if (count < 0) break
                            if (count > maxBytes - total) throw IOException("Download exceeds size limit")
