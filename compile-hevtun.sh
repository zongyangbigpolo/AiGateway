...
HEV_SOURCE="${HEV_SOURCE:-$__dir/.native-build/hev-socks5-tunnel}"
if [[ ! -f "$HEV_SOURCE/Android.mk" ]]; then
  echo "Missing pinned hev sources. Run bash scripts/prepare-native.sh first." >&2
  exit 1
fi
WORKDIR="$__dir/.native-build/hevtun"
mkdir -p "$WORKDIR"
...
ln -sfn "$HEV_SOURCE" jni/hev-socks5-tunnel
...
"$NDK_HOME/ndk-build" \
    ...
    "APP_ABI=$ABIS" \
    APP_PLATFORM=android-24 \
    NDK_LIBS_OUT="$WORKDIR/libs-exec" \
    NDK_OUT="$WORKDIR/obj-exec" \
    "APP_CFLAGS=-O3" \
    "APP_LDFLAGS=-Wl,--build-id=none -Wl,--hash-style=gnu" \
...
for abi in $ABIS; do
  ...
  cp "$WORKDIR/libs/$abi/libhev-socks5-tunnel.so" "$__dir/V2rayNG/app/libs/$abi/"
  cp "$WORKDIR/libs-exec/$abi/hevsockstun" "$__dir/V2rayNG/app/libs/$abi/libhevsockstun.so"
done
